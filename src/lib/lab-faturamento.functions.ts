    ...
    while (fila.length) {
      ...
      try {
        ...
        if (!dry_run) {
          ...
          if (headers.length > 0) {
            const { error } = await supabaseAdmin.from("lab_invoice_header").upsert(headers, { onConflict: "invoice_id" });
            if (error) console.error("[SYNC] Erro ao gravar headers:", error);
          }
          
          if (faturamentos.length > 0) {
            const { error } = await supabaseAdmin.from("lab_faturamento").upsert(faturamentos, { onConflict: "origem,documento_id,item_id" });
            if (error) console.error("[SYNC] Erro ao gravar faturamentos:", error);
          }
          
          if (recebimentos.length > 0) {
            const { error } = await supabaseAdmin.from("lab_recebimento").upsert(recebimentos, { onConflict: "origem,documento_id,pagamento_id" });
