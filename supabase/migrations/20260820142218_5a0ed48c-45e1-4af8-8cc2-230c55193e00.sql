ALTER TABLE public.lab_faturamento DROP CONSTRAINT IF EXISTS lab_faturamento_origem_check;
ALTER TABLE public.lab_faturamento ADD CONSTRAINT lab_faturamento_origem_check CHECK (origem IN ('particular', 'convenio', 'convenio_estimado'));

DROP FUNCTION IF EXISTS public.lab_enriquecer_faturamento();
...
BEGIN
    ...
    WHERE f.agendamento_id = p.agendamento_id
      AND (f.paciente_nome IS NULL OR f.prontuario IS NULL OR f.paciente_nome = '');
