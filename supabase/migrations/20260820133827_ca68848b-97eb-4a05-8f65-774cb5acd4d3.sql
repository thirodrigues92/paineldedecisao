...
BEGIN
    ...
    -- 2. Atualizar nomes de pacientes
    UPDATE public.lab_faturamento f
    SET 
        paciente_nome = COALESCE(NULLIF(f.paciente_nome, ''), sub.nome, sub.nome_paciente),
        prontuario = COALESCE(NULLIF(f.prontuario, ''), sub.prontuario)
    FROM (
        SELECT 
            f2.id,
            p2.nome,
    ...
    UPDATE public.lab_faturamento f
    SET 
        procedimento_nome = COALESCE(NULLIF(f.procedimento_nome, ''), sub.nome, sub.proc_nome),
        grupo_id = sub.grupo_id,
        grupo_nome = sub.grupo_nome
    FROM (
        SELECT 
            f2.id,
            p2.nome,
